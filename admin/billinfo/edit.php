...
if (isset($_GET['billID']) && isset($_GET['prdChillID'])) {
    ...
    $sql = "select * from billinfor where billID = $billID and prdChillID = $prdChillID";
    $billinfor = $db->fetchOne($sql);
    $sql1 = "select prdchill.* , product.name as 'prdName' from prdchill, product where prdchill.prdID = product.id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update = $db->update('billinfor', $data, array('billID' => $billID, 'prdChillID' => $prdChillID));
    if ($update > 0) {
                            ...
                            <div class="basic-form">
                                <form method="POST" action="" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <select id="prdChillIDSelect" onchange="prdchillChange()" name="prdChillID" required class="form-control">
                                                <?php foreach ($prdchill as $item) : ?>
                                                    <?php if ($item['id'] == $billinfor['prdChillID']) : ?>
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            <label>Giá Gốc</label>
                                            <input type="number" required value="<?php echo $item['price'] ?>" name="price" class="form-control">
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label>Giá Sale</label>
                                            <input type="number" required value="<?php echo $item['priceSale'] ?>" name="priceSale" class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
